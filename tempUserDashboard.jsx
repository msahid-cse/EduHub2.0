...
const UserDashboard = () => {
  ...
  // Add state for deletion modal and verification code
  ...
  const handleSaveProfile = async () => {
    try {
      ...
      if (saveBtn) {
        ...
        saveBtn.innerHTML = '<svg class="w-4 h-4 mr-2" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" fill="none"><path d="M5 13l4 4L19 7"></path></svg> Save Profile';
      }
    }
  };
  ...
  const handleSkillProficiencyChange = (e) => {
    const { name, value } = e.target;
    setSkillProficiency(prev => ({ ...prev, [name]: value }));
  };
  ...
  const handleAddSkillWithProficiency = () => {
    ...
    if (profileData.skills.includes(cleanSkill)) {
      ...
      // Create a new skillsMap if it doesn't exist
      const updatedSkillsMap = { ...(profileData.skillsMap || {}) };
      
      // Update or add the skill's proficiency
      updatedSkillsMap[cleanSkill] = {
        proficiency: parseInt(skillProficiency.proficiency, 10),
        lastUpdated: new Date().toISOString()
      };
      
      setProfileData(prev => ({
        ...prev,
        skillsMap: updatedSkillsMap
      }));
      ...
      const updatedSkills = [...profileData.skills, cleanSkill];
      
      // Create a new skillsMap if it doesn't exist
      const updatedSkillsMap = { ...(profileData.skillsMap || {}) };
      
      // Add the new skill's proficiency
      updatedSkillsMap[cleanSkill] = {
        proficiency: parseInt(skillProficiency.proficiency, 10),
        lastUpdated: new Date().toISOString()
      };
      
      // Update profile data with new skill and proficiency
      setProfileData(prev => ({
        ...prev,
            ...
            <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="md:col-span-2 space-y-4">
                      ...
                      {profileData.skills.map((skill, index) => (
                        ...
                        >
                          ...
                          <span className="text-xs text-green-400">
                            {profileData.skillsMap && profileData.skillsMap[skill] 
                              ? `${profileData.skillsMap[skill].proficiency}%` 
                              : '50%'}
                          </span>
                          <button 
                  ...
                  <div className="flex items-center justify-between p-4 bg-gray-800/50 border border-red-800/30 rounded-lg">
                    <div>
                    ...
                    <button 
                      ...
                      className="px-4 py-2 bg-red-600 hover:bg-red-700 rounded-lg text-white text-sm flex items-center"
                    >
...
This appointment request was sent via the EduHub 2.0 Appointment System.
  ...
  // Function to request account deletion verification code
  const requestDeletionCode = async () => {
    try {
      ...
      const response = await axios.post(
        'http://localhost:5000/api/users/request-account-deletion',
        {},
        {
          headers: {
            'Authorization': `Bearer ${token}`
      ...
      {showDeletionModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 rounded-lg shadow-xl w-full max-w-md">
            <div className="p-6">
              <h3 className="text-xl font-semibold text-white mb-4">Delete Your Account</h3>
              ...
              <div className="mb-6">
                ...
                <p className="text-gray-300 text-center">
                  This action <span className="font-bold text-red-500">cannot be undone</span>. All your data, including courses, progress, and personal information will be permanently deleted.
                </p>
              </div>
              ...
              {!isDeletionCodeSent ? (
                <div className="mb-6">
                  <button
                    ...
                    disabled={isDeleting}
                    className="w-full px-4 py-2 bg-red-600 hover:bg-red-700 disabled:bg-gray-600 rounded-lg text-white font-medium flex items-center justify-center"
                  >
                    {isDeleting ? (
                      <>
                ...
                <div className="mb-6">
                  ...
                  <input
                    type="text"
                    value={deletionVerificationCode}
                    onChange={(e) => setDeletionVerificationCode(e.target.value)}
                  ...
                  <button
                    ...
                    disabled={isDeleting}
                    className="w-full mt-4 px-4 py-2 bg-red-600 hover:bg-red-700 disabled:bg-gray-600 rounded-lg text-white font-medium flex items-center justify-center"
                  >
                    {isDeleting ? (
                      <>
                        <span className="animate-spin mr-2">⟳</span> 
